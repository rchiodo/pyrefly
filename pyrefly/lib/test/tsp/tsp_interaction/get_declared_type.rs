...
//! Tests for TSP getDeclaredType request
...
/// Getting the declared type of an annotated variable should succeed.
#[test]
fn test_tsp_get_declared_type_basic() {
    let temp_dir = TempDir::new().unwrap();
    let test_file = temp_dir.path().join("main.py");
    std::fs::write(&test_file, "x: str = \"hello\"\n").unwrap();
    ...
    tsp.server.get_declared_type(&uri, 0, 0, 1);
...
#[test]
fn test_tsp_get_declared_type_snapshot_outdated() {
    let temp_dir = TempDir::new().unwrap();
    let test_file = temp_dir.path().join("main.py");
    ...
    let uri = Url::from_file_path(&test_file).unwrap().to_string();
    tsp.server.get_declared_type(&uri, 0, 0, 0);
